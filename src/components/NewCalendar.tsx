...
export default function NewCalendar() {
  ...
  //const [view, setView] = useState<string>(VIEW_OPTIONS[0].id);
  ...
  const components: any = {
    event: ({ event }: EventProps<EventItem>) => {
      ...
      return <div onClick={()=>setShowPopup(true)} className={`event-${data?.type} h-[26px] rounded-md overflow-hidden text-[12px]  px-[5px] py-[1px] flex justify-between items-center`}>  <div className='flex justify-center items-center gap-2 font-semibold'><span className={`priority-${event?.importance}`}></span> <span>{data?.name}</span></div>  <span>{event.start && formatTimeTo12Hour(event.start)}</span></div>;
    },
        ...
        <div className="flex flex-col md:flex-row justify-between items-center relative px-4 py-4   gap-5">  
            ...
            <div className="flex items-center">
              ...
              <div className="flex flex-col items-start ml-4 w-[290px]">
                  ...
                  <div className="border-[1px] text-[12px] border-[#E9EAEB] py-[2px] px-[4px] h-[27px] rounded-lg w-full flex items-center justify-center  max-w-[80px] shadow-sm"> Week {getWeekNumber(date)}</div>

                </div>

                <div className='text-[14px] text-[#535862] tracking-wider'>{getMonthRange(date)}</div>
              </div>
            ...
            <div className="flex gap-4   md:justify-end  w-[374px]">
              ...
              <div className="flex h-[40px] w-[162px] justify-between border-[1px] rounded-md border-[#E9EAEB] px-1  shadow font-semibold text-[#414651]">
                <button className='mx-2'
                  aria-label="Previous" onClick={onPrevClick}
                  >  <FiArrowLeft /></button>
                <button className="btn px-2 border-[#E9EAEB] border-x-2" onClick={onTodayClick}>Today</button> 
                <button className='mx-2 '
                  aria-label="Next"
                
                  onClick={onNextClick}><FiArrowRight  /></button>
